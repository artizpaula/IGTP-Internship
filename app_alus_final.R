...
data_file <- if (file.exists("data_app.rds")) "data_app.rds" else "data_app(1).rds"
if (!file.exists(data_file)) {
  stop("Missing data file. Expected data_app.rds (or data_app(1).rds).")
}
data <- readRDS(data_file) # keep the data file in the same folder as app_alus.R
...
# vertical dashed lines marking where one chromosome ends and the next begins,
# used by the genome-wide overview plots (chr_boundary_shapes)
chr_boundary_positions <- cumsum(chrom_lengths)
chr_boundary_positions <- chr_boundary_positions[-length(chr_boundary_positions)] # drop the final genome-end boundary
chr_boundary_shapes <- lapply(chr_boundary_positions, function(x) {
  list(type = "line", x0 = x, x1 = x, y0 = 0, y1 = 1, yref = "paper",
       line = list(color = "rgba(150,150,150,0.4)", width = 1, dash = "dot"))
})

# genome-wide tables are prepared lazily below

# Feature x Bin Heatmap: per-patient data is prepared lazily.
# This is intentionally NOT computed at app startup: the merge/aggregate can be
# expensive for the full methylation_long table and can otherwise exceed the
# shinyapps.io worker startup timeout.
.app_cache <- new.env(parent = emptyenv())

get_ml_annot <- function() {
  if (!exists("ml_annot", envir = .app_cache, inherits = FALSE)) {
    bin_lookup <- bin_table[, c("chr", "bin_position", "bin_id")]
    bin_lookup$chr <- as.character(bin_lookup$chr)
    
    ml <- methylation_long
    ml$chr <- as.character(ml$chr)
    
    ml <- merge(ml, bin_lookup, by = c("chr", "bin_position"))
    ml <- merge(ml, metadata[, c("sample_id", "patient_id", "Type")], by = "sample_id")
    assign("ml_annot", ml, envir = .app_cache)
  }
  get("ml_annot", envir = .app_cache, inherits = FALSE)
}

get_patient_bin_shift <- function() {
  if (!exists("patient_bin_shift", envir = .app_cache, inherits = FALSE)) {
    ml <- get_ml_annot()
    pbt <- aggregate(methylation ~ patient_id + bin_id + chr + Type,
                     data = ml, FUN = mean, na.rm = TRUE)
    
    tumor_vals <- pbt[pbt$Type == "Tumor",
                      c("patient_id", "bin_id", "chr", "methylation")]
    normal_vals <- pbt[pbt$Type == "Normal",
                       c("patient_id", "bin_id", "methylation")]
    names(tumor_vals)[4] <- "meth_tumor"
    names(normal_vals)[3] <- "meth_normal"
    
    pbs <- merge(tumor_vals, normal_vals, by = c("patient_id", "bin_id"))
    pbs$shift <- pbs$meth_tumor - pbs$meth_normal
    pbs$bin_id <- factor(pbs$bin_id, levels = bin_table$bin_id)
    
    assign("patient_bin_shift", pbs, envir = .app_cache)
  }
  get("patient_bin_shift", envir = .app_cache, inherits = FALSE)
}

get_bin_stats <- function() {
  if (!exists("bin_stats", envir = .app_cache, inherits = FALSE)) {
    pbs <- get_patient_bin_shift()
    
    # quick per-bin t-test (Tumor vs Normal) done "by hand" with mean/sd
    # instead of t.test() (t.test() is too slow)
    bs <- aggregate(
      shift ~ bin_id,
      data = pbs,
      FUN = function(x) c(
    ...
    bs <- do.call(data.frame, bs)
    names(bs) <- c("bin_id", "mean_shift", "sd_shift", "n")
    bs$bin_id <- as.character(bs$bin_id)
    bs$t_stat <- ifelse(
      bs$n > 1 & bs$sd_shift > 0,
      bs$mean_shift / (bs$sd_shift / sqrt(bs$n)),
      NA_real_
    )
    bs$p_value <- ifelse(
      !is.na(bs$t_stat) & bs$n > 2,
      2 * stats::pt(-abs(bs$t_stat), df = pmax(bs$n - 1, 1)),
      NA_real_
    )
    bs$q_value <- stats::p.adjust(bs$p_value, method = "BH")
    
    assign("bin_stats", bs, envir = .app_cache)
  }
...
get_genome_wide_bins <- function(include_stats = FALSE) {
  if (!exists("genome_wide_bins_base", envir = .app_cache, inherits = FALSE)) {
    gw <- bin_table
    gw$chr <- factor(gw$chr, levels = chrom_list)
    gw <- gw[order(gw$chr, gw$bin_position), ]
    gw$genome_x <- chr_offset[as.character(gw$chr)] + gw$bin_position
    gw$overall_meth <- rowMeans(
      gw[, c("mean_methylation_tumor", "mean_methylation_normal")],
      na.rm = TRUE
    )
    gw$chr_parity <- factor(as.integer(gw$chr) %% 2)
    assign("genome_wide_bins_base", gw, envir = .app_cache)
  }
  
  gw <- get("genome_wide_bins_base", envir = .app_cache, inherits = FALSE)
  if (!include_stats) return(gw)
  
  if (!exists("genome_wide_bins_with_stats", envir = .app_cache, inherits = FALSE)) {
    gw <- merge(
      gw,
      get_bin_stats()[, c("bin_id", "mean_shift", "sd_shift", "n", "p_value", "q_value")],
    ...
    manhattan_outlier_threshold <- stats::quantile(
      abs(gw$mean_diff_tumor_normal), 0.95, na.rm = TRUE
    )
    
    gw$manhattan_category <- with(gw, {
      sig <- !is.na(q_value) & q_value < manhattan_sig_threshold
      outlier <- !is.na(mean_diff_tumor_normal) &
        abs(mean_diff_tumor_normal) >= manhattan_outlier_threshold
      ifelse(
        sig & mean_diff_tumor_normal > 0, "Significant hypermethylation",
        ifelse(
          sig & mean_diff_tumor_normal < 0, "Significant hypomethylation",
          ifelse(outlier, "Extreme outlier", "Not significant")
        )
      )
    })
    gw$manhattan_category <- factor(
      gw$manhattan_category,
      levels = c(
        "Not significant", "Extreme outlier",
        "Significant hypomethylation", "Significant hypermethylation"
      )
    )
    
    assign("genome_wide_bins_with_stats", gw, envir = .app_cache)
  }
  get("genome_wide_bins_with_stats", envir = .app_cache, inherits = FALSE)
}
...
server <- function(input, output, session) {
  ...
  output$genome_overview_plot <- renderPlotly({
    ...
    p <- plot_ly(df, x = ~genome_x, y = ~overall_meth, color = ~chr_parity, colors = c("#16324f", "#0e7c86"), customdata = ~as.character(chr),
      ...
      layout(showlegend = FALSE, dragmode = FALSE, xaxis = list(title = "", tickvals = as.numeric(chr_mid), ticktext = chrom_list, tickangle = 45), yaxis = list(title = "Mean methylation", range = c(0, 1), automargin = TRUE), shapes = chr_boundary_shapes, margin = list(t = 20, r = 20, b = 40, l = 50), autosize = TRUE) |>
      config(displayModeBar = FALSE, responsive = TRUE) |>
      event_register("plotly_click")
  })
  register_plotly_export(output, "genome_overview_plot",
                         function() get_genome_wide_bins(FALSE)[, c("bin_id", "chr", "bin_position", "genome_x", "overall_meth")])
  ...
  browser_view_samples <- reactive({
    ...
    req(nrow(bins_now) > 0)
    df <- get_ml_annot()[get_ml_annot()$bin_id %in% bins_now$bin_id, c("sample_id", "patient_id", "Type", "bin_id", "bin_position", "methylation")]
  ...
  projection_matrix <- reactive({
    ...
    df <- get_ml_annot()[get_ml_annot()$bin_id %in% bins_now, c("sample_id", "bin_id", "methylation")]
  ...
  tn_density_build <- reactive({
    if (view_by_samples()) {
      df <- get_ml_annot()[get_ml_annot()$bin_id %in% selected_bins() & get_ml_annot()$Type %in% c("Tumor", "Normal"), c("Type", "methylation")]
      validate(need(nrow(df) > 0, "No bins selected. Choose a chromosome or bins in the sidebar."))
      plot_df <- data.frame(Type = df$Type, Methylation = df$methylation)
      plot_df <- plot_df[is.finite(plot_df$Methylation), ]
      validate(need(nrow(plot_df) > 0, "No valid methylation values for the current selection."))
  ...
  register_plotly_export(output, "manhattan_plot",
                         function() get_genome_wide_bins(TRUE)[, c("bin_id", "chr", "bin_position", "mean_diff_tumor_normal", "q_value", "n", "manhattan_category")])
  ...
  heatmap_build <- reactive({
    ...
    patient_bin_shift <- get_patient_bin_shift()
  ...
  clinical_boxplot_build <- reactive({
    ...
    if (view_by_samples()) {
      ...
      bins_now <- selected_bins()
      df <- get_ml_annot()[get_ml_annot()$bin_id %in% bins_now & get_ml_annot()$Type == "Tumor", c("sample_id", "patient_id", "bin_id", "methylation")]
