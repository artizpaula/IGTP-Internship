...
library(data.table) # only used (via data.table::) to join the 16M+ row Alu methylation table memory-efficiently
...
# Alu-level data (parallel to the bin-level data above)
data_alus <- readRDS("data_app_alus.rds") # this file has to be in the same folder as app.R
alu_table <- data_alus$alu_table
methylation_long_alus <- data_alus$methylation_long_alus
patient_alu_shift <- data_alus$patient_alu_shift
rm(data_alus)
...
patient_alu_shift$patient_id <- as.character(patient_alu_shift$patient_id)
...
# two-row header for a Bin/Alu Table DT (htmltools to make it fancier)
make_table_header_sketch <- function(columns) {
  col_groups <- vapply(columns, function(c) c$group, character(1))
  ...
  htmltools::withTags(table(
    ...
    tr(lapply(columns, function(c) th(style = "white-space:nowrap;", c$label))))))
}
bin_table_header_sketch <- make_table_header_sketch(bin_table_columns)
...
# `prefix` lets the Alu Table reuse this same builder with its own "alufilter_" input namespace.
...
apply_bin_table_filters <- function(df, filters, input, prefix = "binfilter_") {
  # chromosome filter
  chr_selected <- input[[paste0(prefix, "chr")]]
  ...
  gene_query <- input[[paste0(prefix, "gene")]]
...
# Alu Table tab (parallel to the Bin Table block above)

# Alus are individual repeat elements (tens to hundreds of bp)
format_alu_coordinates <- function(start, end) {
  paste0(format(start, big.mark = ",", scientific = FALSE), "\u2013", format(end, big.mark = ",", scientific = FALSE),
         " (", format(end - start, big.mark = ",", scientific = FALSE), " bp)")
}
alu_table$alu_coordinates <- format_alu_coordinates(alu_table$alu_start, alu_table$alu_end)  # vectorized (945k rows - mapply would be far slower)

# whether any Alu actually ended up with gene/promoter annotation
gene_annotation_available_alus <- any(!is.na(alu_table$gene_names))
all_gene_names_alus <- sort(unique(trimws(unlist(strsplit(na.omit(alu_table$gene_names), ";", fixed = TRUE)))))
promoter_annotation_available_alus <- "promoter_count" %in% names(alu_table) && any(alu_table$promoter_count > 0, na.rm = TRUE)
...
alu_table_columns <- list(list(id = "alu_id", label = "Alu ID", group = "Alu"),
                          ...
                          list(id = "n_present_Tumor", label = "Tumor Samples", group = "Coverage"),
                          list(id = "n_present_Normal", label = "Normal Samples", group = "Coverage"),
                          list(id = "prevalence_total", label = "Prevalence %", group = "Coverage", digits = 3),
                          list(id = "alu_status", label = "Status", group = "Coverage"),
                          list(id = "gene_count", label = "Gene Count", group = "Gene Annotation (COSMIC Cancer Gene Census)"),
                          list(id = "gene_names", label = "Genes", group = "Gene Annotation (COSMIC Cancer Gene Census)"),
                          list(id = "promoter_count", label = "Promoter Count", group = "Promoter Annotation"),
                          list(id = "quick_links_html", label = "Quick Links", group = "External Resources", html = TRUE, plain_id = "quick_links_text"))

alu_table_header_sketch <- make_table_header_sketch(alu_table_columns)
...
alu_table_filters <- list(list(id = "mean_methylation_tumor", label = "Mean Meth (Tumor)", op = ">=", step = 0.01),
                          ...
                          list(id = "n_present_Tumor", label = "Tumor Samples", op = ">=", step = 1),
                          list(id = "n_present_Normal", label = "Normal Samples", op = ">=", step = 1),
                          list(id = "prevalence_total", label = "Prevalence %", op = ">=", step = 0.01),
                          list(id = "gene_count", label = "Gene Count", op = ">=", step = 1),
                          list(id = "promoter_count", label = "Promoter Count", op = ">=", step = 1))
...
# Alu-level counterparts of ml_annot/genome_wide_bins
ml_annot_alus <- {
  dt_long <- data.table::as.data.table(methylation_long_alus)
  dt_meta <- data.table::as.data.table(metadata[, c("sample_id", "patient_id", "Type")])
  data.table::setkey(dt_long, sample_id); data.table::setkey(dt_meta, sample_id)
  dt_long[dt_meta, on = "sample_id", `:=`(patient_id = i.patient_id, Type = i.Type)]
  dt_long <- dt_long[!is.na(Type)]
  out <- as.data.frame(dt_long)
  rm(dt_long, dt_meta); gc()
  out
}
rm(methylation_long_alus); gc()


patient_alu_shift$alu_id <- factor(patient_alu_shift$alu_id, levels = alu_table$alu_id)

# alu_table already carries mean_shift/sd_shift/n/t_stat/p_value/q_value
genome_wide_alus <- alu_table
genome_wide_alus$chr <- factor(genome_wide_alus$chr, levels = chrom_list)
genome_wide_alus <- genome_wide_alus[order(genome_wide_alus$chr, genome_wide_alus$alu_start), ]
genome_wide_alus$genome_x <- chr_offset[as.character(genome_wide_alus$chr)] + genome_wide_alus$alu_start
genome_wide_alus$overall_meth <- rowMeans(genome_wide_alus[, c("mean_methylation_tumor", "mean_methylation_normal")], na.rm = TRUE)
genome_wide_alus$chr_parity <- factor(as.integer(genome_wide_alus$chr) %% 2)

manhattan_sig_threshold_alus <- 0.05
manhattan_outlier_threshold_alus <- stats::quantile(abs(genome_wide_alus$tumor_normal_diff), 0.95, na.rm = TRUE)

genome_wide_alus$manhattan_category <- with(genome_wide_alus, {
  sig <- !is.na(q_value) & q_value < manhattan_sig_threshold_alus
  outlier <- !is.na(tumor_normal_diff) & abs(tumor_normal_diff) >= manhattan_outlier_threshold_alus
  ifelse(sig & tumor_normal_diff > 0, "Significant hypermethylation",
         ifelse(sig & tumor_normal_diff < 0, "Significant hypomethylation",
                ifelse(outlier, "Extreme outlier", "Not significant")))})
genome_wide_alus$manhattan_category <- factor(genome_wide_alus$manhattan_category,
                                              levels = c("Not significant", "Extreme outlier", "Significant hypomethylation", "Significant hypermethylation"))

genome_wide_alus_plot_data <- genome_wide_alus[is.finite(genome_wide_alus$n_present_total) & genome_wide_alus$n_present_total >= 5, ]
...
build_clinical_categorical_filter_inputs <- function(cols, df, compact = FALSE, prefix = "clinfilter_") {
  lapply(cols, function(col) {
    choices <- clinical_categorical_choices(df[[col]])
    if (compact) {
      div(style = "min-width:150px;",
          checkboxGroupInput(inputId = paste0(prefix, col), label = clinical_label_for(col), choices = choices, selected = choices, inline = TRUE))
    } else {
      div(style = "flex: 1 1 220px; min-width: 200px; max-width: 320px;",
          ...
          checkboxGroupInput(inputId = paste0(prefix, col), label = NULL, choices = choices, selected = choices, inline = TRUE))
    }
  })
}

# one range slider per numeric clinical column (age, CNV counts, survival times, methylation summaries, etc.)
build_clinical_numeric_filter_inputs <- function(cols, df, compact = FALSE, prefix = "clinfilter_") {
  lapply(cols, function(col) {
    vals <- df[[col]]
    vals <- vals[is.finite(vals)]
    rng <- if (length(vals) > 0) range(vals) else c(0, 1)
    ...
    div(style = width_style,
        sliderInput(inputId = paste0(prefix, col), label = clinical_label_for(col),
...
apply_clinical_filters <- function(df, numeric_cols, categorical_cols, input, prefix = "clinfilter_") {
  for (col in categorical_cols) {
    sel <- input[[paste0(prefix, col)]]
    if (is.null(sel)) next
...
# turns the long shift table into a patient x bin (or patient x alu) matrix
build_shift_matrix <- function(df, bin_ids, id_col = "bin_id") {
  m <- tapply(df$shift, list(df$patient_id, as.character(df[[id_col]])), FUN = identity)
...
# same idea as parse_genomic_search(), but for the Alu Genome Browser: Alu IDs are "chr_start_end"
# (three parts, exact element coordinates) instead of a bin's "chr_position" (rounded to 1 Mb).
parse_genomic_search_alus <- function(query, alu_table, chrom_list, gene_lookup_table, pad = 2e4) {
  q <- trimws(query)
  if (!nzchar(q)) return(list(status = "error", message = "Please enter a coordinate, Alu ID, or gene name."))
  q_nospace <- gsub(",", "", q)
  ...
  if (grepl(coord_pattern, q_nospace)) {
    ...
    if (end < start) { tmp <- start; start <- end; end <- tmp }
    return(list(status = "ok", chr = chr, start = start, end = end,
                message = paste0("Jumped to chr", chr, ":", format(start, big.mark = ",", scientific = FALSE),
                                 "-", format(end, big.mark = ",", scientific = FALSE))))
  }
  
  # Alu ID format "chr_start_end"
  alu_pattern <- "^([0-9]{1,2}|[XxYy])_([0-9]+)_([0-9]+)$"
  if (grepl(alu_pattern, q_nospace)) {
    m <- regmatches(q_nospace, regexec(alu_pattern, q_nospace))[[1]]
    chr <- toupper(m[2])
    if (chr %in% chrom_list) {
      hit <- alu_table[alu_table$chr == chr & alu_table$alu_start == as.numeric(m[3]) & alu_table$alu_end == as.numeric(m[4]), ]
      if (nrow(hit) == 1) {
        return(list(status = "ok", chr = chr, start = max(1, hit$alu_start - pad), end = hit$alu_end + pad,
                    message = paste0("Jumped to Alu ", hit$alu_id)))
      }
    }
  }
  
  gene_query <- toupper(q)
  has_annotation <- !is.na(alu_table$gene_names) &
    grepl(paste0("(^|[,;])\\s*", gene_query, "\\s*($|[,;])"), toupper(alu_table$gene_names))
  in_annotation <- alu_table[has_annotation, ]
  if (nrow(in_annotation) >= 1) {
    return(list(status = "ok", chr = in_annotation$chr[1],
                start = min(in_annotation$alu_start), end = max(in_annotation$alu_end),
                message = paste0("Jumped to gene ", gene_query)))
  }
  ...
  if (nrow(in_lookup) == 1) {
    return(list(status = "ok", chr = in_lookup$chr,
                start = max(1, in_lookup$start - 2e6), end = in_lookup$end + 2e6,
                message = paste0("Jumped to ", gene_query, " (", in_lookup$chr, ":",
                                 format(in_lookup$start, big.mark = ",", scientific = FALSE), "-",
                                 format(in_lookup$end, big.mark = ",", scientific = FALSE), ")")))
  }
  list(status = "error", message = paste0("'", q, "' was not recognized as a coordinate, Alu ID, or gene name."))
}
...
# same idea as parse_bin_file(), for uploading a list of alu_id values ("chr_start_end")
parse_alu_file <- function(filepath, filename, alu_table) {
  ext <- tolower(tools::file_ext(filename))
  raw_ids <- character(0)
  tbl <- NULL
  if (ext %in% c("csv", "tsv")) {
    sep <- if (ext == "tsv") "\t" else ","
    tbl <- tryCatch(utils::read.csv(filepath, sep = sep, header = TRUE, stringsAsFactors = FALSE, check.names = FALSE),
                    error = function(e) NULL)
  }
  if (!is.null(tbl) && ncol(tbl) >= 1 && nrow(tbl) >= 1) {
    nm <- tolower(trimws(names(tbl)))
...
# same Quick Links columns for the Alu Table
alu_chr_lab <- paste0("chr", alu_table$chr)
alu_start_fmt <- format(round(alu_table$alu_start), scientific = FALSE, trim = TRUE)
alu_end_fmt <- format(round(alu_table$alu_end), scientific = FALSE, trim = TRUE)
alu_ucsc_url <- paste0("https://genome.ucsc.edu/cgi-bin/hgTracks?db=hg19&position=", alu_chr_lab, "%3A", alu_start_fmt, "-", alu_end_fmt)
alu_ensembl_url <- paste0("https://grch37.ensembl.org/Homo_sapiens/Location/View?r=", alu_table$chr, "%3A", alu_start_fmt, "-", alu_end_fmt)
alu_tcga_url <- "https://portal.gdc.cancer.gov/exploration?filters=%7B%22op%22%3A%22in%22%2C%22content%22%3A%7B%22field%22%3A%22cases.project.project_id%22%2C%22value%22%3A%5B%22TCGA-COAD%22%2C%22TCGA-READ%22%5D%7D%7D"
alu_table$quick_links_html <- paste0(
  '<a href="', alu_ucsc_url, '" target="_blank" rel="noopener noreferrer" style="margin-right:10px; font-size:12px; color:#2b6cb0; text-decoration:none; white-space:nowrap;">\u2197 UCSC</a>',
  '<a href="', alu_ensembl_url, '" target="_blank" rel="noopener noreferrer" style="margin-right:10px; font-size:12px; color:#0e7c86; text-decoration:none; white-space:nowrap;">\u2197 Ensembl</a>',
  '<a href="', alu_tcga_url, '" target="_blank" rel="noopener noreferrer" style="font-size:12px; color:#d1495b; text-decoration:none; white-space:nowrap;">\u2197 TCGA</a>')
alu_table$quick_links_text <- paste0("UCSC: ", alu_ucsc_url, " | Ensembl: ", alu_ensembl_url, " | TCGA: ", alu_tcga_url)
rm(alu_chr_lab, alu_start_fmt, alu_end_fmt, alu_ucsc_url, alu_ensembl_url, alu_tcga_url)
...
# Names of the new Alu-resolution tabs, used to switch the sidebar between the Bin controls and the Alu controls 
alu_tab_names <- c("Overview (Alus)", "Genome Browser (Alus)", "Genome-wide Profile (Alus)",
                   "Feature x Chromosome Heatmap (Alus)", "Clinical Explorer (Alus)", "Alu Table", "Tumor vs. Normal (Alus)")
alu_tab_js_array <- paste0("[", paste0("'", gsub("'", "\\\\'", alu_tab_names), "'", collapse = ","), "]")
is_alu_tab_js <- paste0(alu_tab_js_array, ".indexOf(input.main_nav) > -1")
...
ui <- page_sidebar(title = div(style = "display:flex; justify-content:space-between; align-items:center; width:100%; padding:18px 10px; border-bottom:1px solid rgba(255,255,255,0.10);",
                   ...
                   sidebar = sidebar(width = 300,
                                     ...
                                     conditionalPanel(paste0("input.main_nav != 'Home' && !(", is_alu_tab_js, ")"),
                                                      ...
                                                      hr(style = "margin:1px 0; border-color:#3a5470;")),
                                     # Alu-selection controls, shown only on the new Alu-resolution tabs - mirrors the bin controls above 1:1.
                                     conditionalPanel(paste0("(", is_alu_tab_js, ")"),
                                                      tags$div("DATA SELECTION (ALUS)", style = "font-size:14px; font-weight:700; letter-spacing:0.8px; color:#7d92a3; margin-bottom:0px;"),
                                                      selectInput("chr_alus", "Chromosome:", choices = chrom_list),
                                                      selectizeInput("bins_alus", "Selected Alus:", choices = NULL, multiple = TRUE,
                                                                     options = list(placeholder = "Find and select Alus (ex: 1_51584_51880)...", plugins = list("remove_button"), maxOptions = 500)),
                                                      actionButton("add_chr_alus", "Add all Alus on the chromosome", icon = bs_icon("plus-circle"), class = "btn-sm class=btn-outline-light w-100"),
                                                      actionButton("clear_alus", "Clear Alu selection", icon = bs_icon("x-circle"), class = "btn-sm class=btn-outline-light w-100"),
                                                      ...
                                                      tags$div("Accepts .csv/.tsv/.txt or a plain list of Alu IDs like 1_51584_51880, one per line or comma-separated. Each Alu is a single repeat element (tens-hundreds of bp), and per-element coverage is sparse, so select several to get usable statistics.",
                                                               style = "font-size:11px; color:#7d92a3; margin-top:-10px; margin-bottom:8px;"),
                                                      hr(style = "margin:1px 0; border-color:#3a5470;"),
                                                      tags$div("VISUALIZATION LEVEL", style = "font-size:14px; font-weight:700; letter-spacing:0.8px; color:#7d92a3; margin-bottom:0px;"),
                                                      radioButtons("view_level_alus", NULL,
                                                                   choices = c("By Alu (aggregated)" = "bins", "By Sample (raw)" = "samples"),
                                                                   selected = "bins", inline = TRUE),
                                                      tags$div("Applies to plots where both views make sense: switches between pre-aggregated per-Alu means and unaggregated per-sample methylation values.",
                                                               style = "font-size:11px; color:#7d92a3; margin-top:-10px; margin-bottom:8px;"),
                                                      hr(style = "margin:1px 0; border-color:#3a5470;"))),
                   ...
                   navset_card_underline(title = "Exploration and Visualization", id = "main_nav",
                                         ...
                                         # ---- Alu-resolution tabs (parallel to the Bin-resolution tabs above) ----
                                         
                                         # Alu 1. Overview
                                         nav_panel("Overview (Alus)", div(style = "font-size:13px; color:#6c757d; margin-bottom:10px;",
                                                                          "Same four summary plots as the bin-level Overview, but computed from the Alu(s) currently selected in the sidebar. Alu-level coverage is much sparser than bin-level coverage (each element is only ~300 bp), so select several Alus to get a usable sample size."),
                                                   uiOutput("overview_content_alus")),
                                         
                                         # Alu 2. Genome Browser
                                         nav_panel("Genome Browser (Alus)",
                                                   div(style = "font-size:13px; color:#6c757d; margin-bottom:10px;",
                                                       "Browse individual Alu elements across the genome: pick a chromosome, zoom in/out, or search by coordinates, Alu ID, or gene name."),
                                                   card(full_screen = TRUE,
                                                        card_header("All chromosomes, click a point to jump to that chromosome"),
                                                        div(class = "gb-plot-wrap", plotlyOutput("genome_overview_plot_alus", height = "calc(max(320px, min(40vh, 420px)))", width = "100%")),
                                                        plotly_export_ui("genome_overview_plot_alus")),
                                                   card(full_screen = TRUE,
                                                        card_header(textOutput("browser_position_header_alus", inline = TRUE)),
                                                        div(class = "gb-toolbar",
                                                            div(class = "gb-toolbar-group gb-search-group",
                                                                textInput("browser_search_alus", label = NULL, placeholder = "e.g. 12:25000000-26000000, 12_25123456_25123750, or KRAS", width = "100%"),
                                                                actionButton("browser_search_go_alus", NULL, icon = bs_icon("search"), class = "btn-sm btn-outline-secondary")),
                                                            div(class = "gb-toolbar-group gb-nav-group",
                                                                actionButton("prev_chr_alus", NULL, icon = bs_icon("chevron-left"), class = "btn-sm btn-outline-secondary"),
                                                                selectInput("browser_chr_alus", NULL, choices = chrom_list, selected = "1", width = "90px"),
                                                                actionButton("next_chr_alus", NULL, icon = bs_icon("chevron-right"), class = "btn-sm btn-outline-secondary")),
                                                            div(class = "gb-toolbar-group gb-zoom-group",
                                                                actionButton("zoom_in_alus", NULL, icon = bs_icon("zoom-in"), class = "btn-sm btn-outline-secondary"),
                                                                actionButton("zoom_out_alus", NULL, icon = bs_icon("zoom-out"), class = "btn-sm btn-outline-secondary"),
                                                                actionButton("zoom_reset_alus", "Whole chromosome", icon = bs_icon("arrow-counterclockwise"), class = "btn-sm btn-outline-secondary"))),
                                                        div(class = "gb-plot-wrap", plotlyOutput("browser_chr_plot_alus", height = "calc(max(440px, min(72vh, 760px)))", width = "100%")),
                                                        plotly_export_ui("browser_chr_plot_alus"),
                                                        plot_desc("Tumor vs Normal methylation for individual Alu elements across one chromosome. When zoomed far out, only elements with data are drawn (most Alus have none in a given sample)."),
                                                        tags$div("Tip: Alus currently in your sidebar selection are outlined on the plot above. Drag on the plot to zoom in, double-click to reset, or use the range slider below the plot to pan across the chromosome.",
                                                                 style = "font-size:11px; color:#7d92a3; margin-top:6px;")),
                                                   card(card_header("External Resources for This Region"), uiOutput("browser_external_links_alus"))),
                                         
                                         # Alu 3. Genome-wide Profile
                                         nav_panel("Genome-wide Profile (Alus)", div(style = "font-size:13px; color:#6c757d; margin-bottom:10px;",
                                                                                     "Genome-wide Tumor vs. Normal methylation across all individually-covered Alu elements. Significant Alus (q < 0.05) and top 5% extreme differences are flagged. Hover for details; drag to zoom, double-click to reset, and use the camera icon to export."),
                                                   card(card_header("Manhattan-style plot: Tumor \u2212 Normal methylation difference (Alus)"), plotlyOutput("manhattan_plot_alus", height = "calc(max(360px, min(58vh, 620px)))"), plotly_export_ui("manhattan_plot_alus"), plot_desc("Every Alu's Tumor\u2212Normal difference across the whole genome, colours mark statistically significant and outlier elements."),
                                                        div(style = "display:flex; gap:20px; flex-wrap:wrap; margin-top:12px; padding-top:10px; border-top:1px solid #e3e8ec; font-size:12px; color:#3a5470;",
                                                            tags$span(tags$span(style = "display:inline-block; width:10px; height:10px; border-radius:50%; background:#8D99AE; margin-right:6px;"), "Significant hypermethylation (q < 0.05)"),
                                                            tags$span(tags$span(style = "display:inline-block; width:10px; height:10px; border-radius:50%; background:#76C893; margin-right:6px;"), "Significant hypomethylation (q < 0.05)"),
                                                            tags$span(tags$span(style = "display:inline-block; width:10px; height:10px; border-radius:50%; background:#F4A261; margin-right:6px;"), "Extreme outlier (top 5% |\u0394|, not significant)"),
                                         ...
                                         # Alu 4. Feature x Chromosome Heatmap
                                         nav_panel("Feature x Chromosome Heatmap (Alus)", card(full_screen = TRUE, card_header("Diverging heatmap of methylation shift (feature \u00d7 Alu)"),
                                                                                               div(style = "font-size:13px; color:#6c757d; margin-bottom:10px;",
                                                                                                   "Same clustered heatmap as the bin-level version, restricted to the Alus currently selected in the sidebar (a 945k-Alu heatmap isn't renderable, so this always reflects your selection - unlike the bin heatmap, which defaults to the whole chromosome). Because a given patient rarely has both a Tumor and a Normal value at every selected Alu, only patients with complete data across the selection are shown; select more Alus if too few remain."),
                                                                                               div(style = "display:flex; align-items:center; gap:14px; flex-wrap:wrap; margin-bottom:10px;",
                                                                                                   div(style = "min-width:220px;", selectInput("heatmap_feature_alus", "Colour strip (clinical feature):", choices = heatmap_feature_choices, selected = "none")),
                                                                                                   div(style = "flex:1 1 200px; min-width:180px; display:flex; align-items:center; gap:8px;",
                                                                                                       tags$span(style="font-size:12px; color:#7d92a3; white-space:nowrap;","Zoom:"),
                                                                                                       sliderInput("heatmap_zoom_alus", NULL, min = 10, max = 400, value = 100, step = 5, post = "%", width = "100%"),
                                                                                                       actionButton("heatmap_zoom_reset_alus", "Fit to screen", icon = bs_icon("aspect-ratio"), class = "btn-sm btn-outline-secondary")) ),
                                                                                               div(id = "heatmap_scroll_wrap_alus", style = "overflow:auto; border:1px solid #e3e8ec; border-radius:8px; padding:10px; max-height:72vh;",
                                                                                                   plotOutput("feature_heatmap_alus", height = "auto")),
                                                                                               plot_export_ui("feature_heatmap_alus"))),
                                         
                                         # Alu 5. Clinical Explorer
                                         nav_panel("Clinical Explorer (Alus)", div(style = "font-size:13px; color:#6c757d; margin-bottom:10px;",
                                                                                   "Same clinical cross-referencing as the bin-level Clinical Explorer, driven by the Alu(s) selected in the sidebar."),
                                                   div(style = "font-size:12px; color:#7d92a3; margin-bottom:12px;", uiOutput("selected_patient_banner_alus")),
                                                   card(card_header("Tumor methylation vs. mutation status (Alus)"),
                                                        div(style = "display:flex; flex-wrap:wrap; gap:18px; align-items:flex-end; margin-bottom:10px;",
                                                            div(style = "min-width:160px;", selectInput("mutation_gene_alus", "Gene:", choices = c("KRAS", "BRAF", "TP53"))),
                                                            div(style = "min-width:220px;", radioButtons("mutation_stat_test_alus", "Statistical test:", choices = c("Wilcoxon rank-sum" = "wilcox", "Welch's t-test" = "ttest"), selected = "wilcox", inline = TRUE))),
                                                        plotOutput("clinical_boxplot_alus", height = "calc(max(300px, min(46vh, 460px)))"), plot_export_ui("clinical_boxplot_alus"), plot_desc("Compares mutant vs wild-type methylation, with a p-value (Wilcoxon or Welch's t-test), among patients currently matching the filters below.")),
                                                   card(card_header(div(style = "display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px;",
                                                                       div(style = "display:flex; align-items:center; gap:8px;", bs_icon("table", size = "1.05em"), "Clinical metadata explorer"),
                                                                       uiOutput("clinical_table_count_badge_alus"))),
                                                       div(style = "display:flex; flex-wrap:wrap; gap:10px 16px; align-items:center; background:#f7f9fa; border:1px solid #e7ecef; border-radius:8px; padding:8px 12px; margin-bottom:10px;",
                                                           tags$span(style = "font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:0.4px; color:#7d92a3; display:flex; align-items:center; gap:4px;", bs_icon("sliders", size = "0.9em"), "Filter:"),
                                                           build_clinical_categorical_filter_inputs(clinical_categorical_cols, clinical_profile_table, compact = TRUE, prefix = "clinfilter_alus_"),
                                                           build_clinical_numeric_filter_inputs(clinical_numeric_cols, clinical_profile_table, compact = TRUE, prefix = "clinfilter_alus_"),
                                                           actionButton("clinfilter_reset_alus", "Reset", icon = bs_icon("arrow-counterclockwise"), class = "btn-sm btn-outline-danger", style = "font-size:11px; padding:3px 10px; border-radius:5px; margin-left:auto;")),
                                                       div(style = "font-size:12px; color:#7d92a3; margin-bottom:8px;", "One row per patient. Click a row to highlight that patient in the plot above."),
                                                       DTOutput("clinical_table_alus"), plot_desc("A searchable table of patient info, click a row to highlight them."))),
                                         ...
                                         nav_panel("Alu Table", card(card_header(div(style = "display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px;",
                                                                                     div(style = "display:flex; align-items:center; gap:8px;", bs_icon("table", size = "1.05em"), "Filterable Alu-level Data"),
                                                                     ...
                                                                     tags$details(style = "background:#ffffff; border:1px solid #d8e0e6; border-radius:10px; padding:0; margin-bottom:20px; box-shadow:0 1px 4px rgba(22,50,79,0.08); overflow:hidden;",
                                                                                  ...
                                                                                  div(style = "padding:18px 20px 20px 20px;",
                                                                                      div(style = "margin-bottom:16px;",
                                                                                          div(style = "display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px; margin-bottom:8px;",
                                                                                              tags$span(style = "font-weight:700; font-size:11.5px; text-transform:uppercase; letter-spacing:0.6px; color:#7d92a3; display:flex; align-items:center; gap:6px;", bs_icon("bar-chart-steps"), "Chromosome"),
                                                                                          ...
                                                                                          div(style = "background:#f7f9fa; border:1px solid #e7ecef; border-radius:8px; padding:10px 12px;", checkboxGroupInput("alufilter_chr", label = NULL, choices = chrom_list, selected = chrom_list, inline = TRUE))),
                                                                                      tags$hr(style = "border-top:1px solid #eef2f5; margin:14px 0;"),
                                                                                      tags$div(style = "font-weight:700; font-size:11.5px; text-transform:uppercase; letter-spacing:0.6px; color:#7d92a3; margin-bottom:10px; display:flex; align-items:center; gap:6px;", bs_icon("sliders2"), "Methylation & Coverage Metrics"),
                                                                                      div(style = "display:flex; flex-wrap:wrap; gap:16px; align-items:flex-start;",
                                                                                          build_bin_table_filter_inputs(alu_table_filters, alu_table, prefix = "alufilter_")),
                                                                                      tags$hr(style = "border-top:1px solid #eef2f5; margin:18px 0 14px 0;"),
                                                                                      div(style = "display:flex; justify-content:flex-end;", actionButton("alufilter_reset", "Reset all filters", icon = bs_icon("arrow-counterclockwise"), class = "btn-sm btn-outline-danger", style = "font-size:12px; border-radius:6px; padding:5px 14px;")))),
                                                                     div(style = "display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px; margin-bottom:15px;", downloadButton("alutable_download", "Download CSV Data", icon = icon("download"), class = "btn-lg btn-primary", style = "font-weight: 600; padding: 10px 22px; font-size: 15px; border-radius: 6px;"),
                                                                         uiOutput("alutable_gene_note"), uiOutput("alutable_promoter_note")),
                                                                     div(style = "margin-top:4px;", DTOutput("alutable")))),
                                         
                                         # Alu 7. Tumor vs Normal
                                         nav_panel("Tumor vs. Normal (Alus)", navset_tab(
                                           nav_panel("Methylation density", card(full_screen = TRUE, card_header("Methylation density (Tumor vs Normal, Alus)"), plotOutput("tn_density_alus", height = "58vh"), plot_export_ui("tn_density_alus"), plot_desc("Shape of the methylation distribution, Tumor vs Normal overlapped, for the selected Alus."))),
                                           nav_panel("Patient similarity network", card(full_screen = TRUE, card_header("Patient similarity network (Alus)"),
                                                                                        div(style = "display:flex; flex-wrap:wrap; gap:18px; align-items:flex-end; margin-bottom:10px;",
                                                                                            div(style = "min-width:260px; max-width:420px; flex:1 1 300px;",
                                                                                                selectizeInput("network_color_feature_alus", "Colour nodes by (pick one or more):", choices = network_color_choices, selected = "Type", multiple = TRUE, options = list(plugins = list("remove_button")))),
                                                                                            div(style = "min-width:240px; max-width:340px; flex:1 1 260px;",
                                                                                                sliderInput("network_corr_threshold_alus", "Minimum correlation for an edge (corr \u2265):", min = -1, max = 1, value = 0.7, step = 0.01, width = "100%"))),
                                                                                        plotOutput("tn_network_alus", height = "54vh"),
                                                                                        plot_export_ui("tn_network_alus"),
                                                                                        plot_desc("Each dot is one sample (Tumor or Normal): a circle for Tumor and a triangle for Normal, restricted to samples with data at every selected Alu. Lines connect sample pairs whose methylation profiles correlate at or above the chosen threshold."))))),
...
server <- function(input, output, session) {
  ...
  output$browser_chr_plot <- renderPlotly({
    if (view_by_samples()) {
      ...
      # highlight only bins explicitly picked in the sidebar's "Selected bins"
  ...
  # Sidebar: Alu selection controls (mirrors the "Selected bins" controls)
  alus_choices_for_chr <- function(chr, extra = character(0)) union(alu_choices_by_chr[[chr]], extra)
  updateSelectizeInput(session, "bins_alus", choices = alus_choices_for_chr(isolate(input$chr_alus)), selected = character(0), server = TRUE)
  
  observeEvent(input$chr_alus, {
    updateSelectizeInput(session, "bins_alus", choices = alus_choices_for_chr(input$chr_alus, input$bins_alus), selected = input$bins_alus, server = TRUE)
  }, ignoreInit = TRUE)
  
  observeEvent(input$add_chr_alus, {
    chr_alus_now <- alu_choices_by_chr[[input$chr_alus]]
    updated <- union(input$bins_alus, chr_alus_now)
    updateSelectizeInput(session, "bins_alus", choices = alus_choices_for_chr(input$chr_alus, updated), selected = updated, server = TRUE)
  })
  
  observeEvent(input$clear_alus, {
    updateSelectizeInput(session, "bins_alus", choices = alus_choices_for_chr(input$chr_alus), selected = character(0), server = TRUE)
  })
  
  observeEvent(input$alus_file, {
    req(input$alus_file)
    res <- parse_alu_file(input$alus_file$datapath, input$alus_file$name, alu_table)
    if (length(res$matched) == 0) {
      showNotification("Couldn't match any Alus in file.", type = "error", duration = 7)
      return()
    }
    updated <- union(input$bins_alus, res$matched)
    updateSelectizeInput(session, "bins_alus", choices = alus_choices_for_chr(input$chr_alus, updated), selected = updated, server = TRUE)
    n_unmatched <- length(res$unmatched)
    if (n_unmatched == 0) {
      showNotification(sprintf("All %d Alu IDs in the file matched and were added to your selection.", length(res$matched)), type = "message", duration = 7)
    } else {
      preview <- paste(utils::head(res$unmatched, 10), collapse = ", ")
      more <- if (n_unmatched > 10) sprintf(" (+%d more)", n_unmatched - 10) else ""
      showNotification(sprintf("Selected %d matching Alus. %d ID(s) from the file don't exist in this dataset and were skipped: %s%s",
                               length(res$matched), n_unmatched, preview, more), type = "warning", duration = 15)
    }
  })
  
  selected_alus <- reactive({ if (length(input$bins_alus) > 0) input$bins_alus else character(0) })
  selected_alu_table <- reactive({ alu_table[alu_table$alu_id %in% selected_alus(), ] })
  view_by_samples_alus <- reactive({ identical(input$view_level_alus, "samples") })
  
  region_label_alus <- reactive({
    df <- selected_alu_table()
    n <- nrow(df)
    chrs <- unique(as.character(df$chr))
    chr_txt <- if (length(chrs) > 0) paste0(" (chr", paste(chrs, collapse = ", chr"), ")") else ""
    paste0("Selected ", n, " Alu", if (n != 1) "s" else "", chr_txt)
  })
  
  # Alu Table filter panel
  observeEvent(input$alufilter_chr_all, { updateCheckboxGroupInput(session, "alufilter_chr", selected = chrom_list) })
  observeEvent(input$alufilter_chr_clear, { updateCheckboxGroupInput(session, "alufilter_chr", selected = character(0)) })
  observeEvent(input$alufilter_reset, {
    ...
    for (f in alu_table_filters) {
      vals <- alu_table[[f$id]]; vals <- vals[is.finite(vals)]
      step <- if (!is.null(f$step)) f$step else 0.01
      lo <- if (length(vals) > 0) { if (step >= 1) floor(min(vals)) else floor(min(vals) * 100) / 100 } else 0
      updateSliderInput(session, paste0("alufilter_", f$id), value = lo)
    }
  })
  filtered_alu_table <- reactive({ apply_bin_table_filters(alu_table, alu_table_filters, input, prefix = "alufilter_") })
  output$alutable_count_badge <- renderUI({
    n <- nrow(filtered_alu_table()); total <- nrow(alu_table)
    span(style = "font-size:12px; font-weight:600; color:#16324f; background:#eef2f5; border:1px solid #d8e0e6; border-radius:20px; padding:4px 12px; white-space:nowrap;",
         paste0(format(n, big.mark = ","), " of ", format(total, big.mark = ","), " Alus"))
  })
  
  # Alu 1. Overview
  overview_ml_selected_alus <- reactive({
    validate(need(length(selected_alus()) > 0, "No Alus selected. Choose a chromosome and Alus in the sidebar (\"Data Selection (Alus)\")."))
    ml_annot_alus[ml_annot_alus$alu_id %in% selected_alus(), ]
  })
  
  overview_patient_shift_alus <- reactive({
    df <- overview_ml_selected_alus()
    validate(need(nrow(df) > 0, "No methylation data available for the selected Alus."))
    agg <- aggregate(methylation ~ patient_id + Type, data = df, FUN = mean, na.rm = TRUE)
    wide <- reshape(agg, idvar = "patient_id", timevar = "Type", direction = "wide")
    names(wide) <- sub("^methylation\\.", "", names(wide))
    if (!"Tumor" %in% names(wide)) wide$Tumor <- NA_real_
    if (!"Normal" %in% names(wide)) wide$Normal <- NA_real_
    wide$shift <- wide$Tumor - wide$Normal
    merge(wide, patient_annotation, by = "patient_id")
  })
  
  overview_plot_meta_alus <- list(
    list(id = "overview_boxplot_alus", title = "Tumor vs. Normal Methylation (Alus)", desc = "Shows if tumors are more or less methylated than normals, at the selected Alus."),
    list(id = "overview_mutations_alus", title = "Mutation Status vs. Methylation Shift (Alus)", desc = "Shows if having a KRAS/BRAF/TP53 mutation changes methylation shift."),
    list(id = "overview_composition_alus", title = "Stage & MSI/MSS vs. Methylation Shift (Alus)", desc = "Shows if shift differs by stage or MSS status."),
    list(id = "overview_sex_comparison_alus", title = "Sex vs. Methylation Shift (Alus)", desc = "Shows if methylation shift differs between males and females."))
  
  overview_zoom_alus <- reactiveVal(NULL)
  lapply(overview_plot_meta_alus, function(pm) {
    observeEvent(input[[paste0("expand_", pm$id)]], { overview_zoom_alus(pm$id) }, ignoreInit = TRUE)
    observeEvent(input[[paste0(pm$id, "_click")]], { overview_zoom_alus(pm$id) }, ignoreInit = TRUE)
  })
  observeEvent(input$overview_back_alus, { overview_zoom_alus(NULL) }, ignoreInit = TRUE)
  
  output$overview_content_alus <- renderUI({
    zoom <- overview_zoom_alus()
    if (is.null(zoom)) {
      card_list <- lapply(overview_plot_meta_alus, function(pm) {
        card(card_header(div(style = "display:flex; justify-content:space-between; align-items:center; width:100%;", tags$span(pm$title),
                             actionButton(paste0("expand_", pm$id), NULL, icon = bs_icon("arrows-fullscreen"), class = "btn-sm btn-outline-secondary", title = "Expand", style = "padding:2px 7px; margin-left:auto; margin-right:-5px;"))),
             plotOutput(pm$id, height = "calc(max(230px, min(34vh, 320px)))", click = paste0(pm$id, "_click")),
             plot_export_ui(pm$id), plot_desc(pm$desc))
      })
      do.call(layout_columns, c(list(col_widths = breakpoints(sm = 12, md = 6), row_heights = "auto"), card_list))
    } else {
      meta <- Filter(function(pm) identical(pm$id, zoom), overview_plot_meta_alus)[[1]]
      tagList(div(style = "margin-bottom:10px;", actionButton("overview_back_alus", "Go back to Overview", icon = bs_icon("arrow-left-circle"), class = "btn-sm btn-outline-primary")),
              card(card_header(meta$title), plotOutput(meta$id, height = "calc(max(360px, min(64vh, 640px)))"), plot_export_ui(meta$id)))
    }
  })
  
  overview_boxplot_build_alus <- reactive({
    if (view_by_samples_alus()) {
      df <- overview_ml_selected_alus()
      plot_df <- data.frame(Type = df$Type, Methylation = df$methylation)
      plot_df <- plot_df[is.finite(plot_df$Methylation) & plot_df$Type %in% c("Normal", "Tumor"), ]
      validate(need(nrow(plot_df) > 0, "No valid methylation values for the current selection."))
      y_lab <- "Methylation (per sample \u00d7 Alu)"
    } else {
      df <- selected_alu_table()
      validate(need(nrow(df) > 0, "No Alus selected. Choose Alus in the sidebar."))
      plot_df <- data.frame(Type = rep(c("Normal", "Tumor"), each = nrow(df)), Methylation = c(df$mean_methylation_normal, df$mean_methylation_tumor))
      plot_df <- plot_df[is.finite(plot_df$Methylation), ]
      validate(need(nrow(plot_df) > 0, "No valid methylation values for the current selection."))
      y_lab <- "Mean methylation (per Alu)"
    }
    plot_df$Type <- factor(plot_df$Type, levels = c("Normal", "Tumor"))
    p <- ggplot(plot_df, aes(x = Type, y = Methylation, fill = Type)) +
      geom_jitter(width = 0.07, size = 0.9, alpha = 0.25, color = "#16324f") +
      ...
      scale_fill_manual(values = c("Tumor" = "#d1495b", "Normal" = "#3aa9c9")) +
      scale_y_continuous(limits = c(0, 1), expand = expansion(mult = c(0.02, 0.06))) +
      labs(x = NULL, y = y_lab, subtitle = region_label_alus(), caption = "\u25c7 mean") +
      theme_app() + theme(legend.position = "none")
    list(plot = p, data = plot_df)
  })
  output$overview_boxplot_alus <- renderPlot({ overview_boxplot_build_alus()$plot })
  register_plot_export(output, "overview_boxplot_alus", function() overview_boxplot_build_alus()$plot, function() overview_boxplot_build_alus()$data)
  
  overview_mutations_build_alus <- reactive({
    df <- overview_patient_shift_alus()
    genes <- c("KRAS", "BRAF", "TP53")
    long_df <- do.call(rbind, lapply(genes, function(g) data.frame(Gene = g, Status = as.character(df[[g]]), Shift = df$shift, stringsAsFactors = FALSE)))
    long_df <- long_df[!is.na(long_df$Status) & is.finite(long_df$Shift), ]
    validate(need(nrow(long_df) > 0, "No mutation status data available for the current selection."))
    status_labels <- c("0" = "Wild-type", "1" = "Mutant")
    long_df$Status <- factor(ifelse(long_df$Status %in% names(status_labels), status_labels[long_df$Status], "Unknown"), levels = c("Wild-type", "Mutant", "Unknown"))
    n_by_gene <- table(long_df$Gene)
    long_df$GeneLabel <- factor(paste0(long_df$Gene, " (n=", n_by_gene[long_df$Gene], ")"), levels = paste0(genes, " (n=", n_by_gene[genes], ")"))
    p <- ggplot(long_df, aes(x = Status, y = Shift, fill = Status)) +
      geom_hline(yintercept = 0, color = "#7d92a3", linewidth = 0.4, linetype = "dashed") +
      geom_jitter(width = 0.1, size = 0.9, alpha = 0.3, color = "#16324f") +
      geom_boxplot(width = 0.5, outlier.shape = NA, alpha = 0.92, color = "#16324f", linewidth = 0.4) +
      scale_fill_manual(values = c("Wild-type" = "#3aa9c9", "Mutant" = "#d1495b", "Unknown" = "darkgray")) +
      facet_wrap(~ GeneLabel, nrow = 1, scales = "free_x") +
      labs(x = NULL, y = "Methylation shift (Tumor \u2212 Normal)", subtitle = region_label_alus()) +
      theme_app() + theme(legend.position = "none", axis.text.x = element_text(angle = 20, hjust = 1))
    list(plot = p, data = long_df[, c("Gene", "Status", "Shift")])
  })
  output$overview_mutations_alus <- renderPlot({ overview_mutations_build_alus()$plot })
  register_plot_export(output, "overview_mutations_alus", function() overview_mutations_build_alus()$plot, function() overview_mutations_build_alus()$data)
  
  overview_composition_build_alus <- reactive({
    df <- overview_patient_shift_alus()
    feats <- c("estadi2", "MSS")
    long_df <- do.call(rbind, lapply(feats, function(f) data.frame(Feature = f, Level = as.character(df[[f]]), Shift = df$shift, stringsAsFactors = FALSE)))
    long_df <- long_df[!is.na(long_df$Level) & is.finite(long_df$Shift), ]
    validate(need(nrow(long_df) > 0, "No stage/MSS data available for the current selection."))
    agg <- aggregate(Shift ~ Feature + Level, data = long_df, FUN = mean)
    n_df <- aggregate(Shift ~ Feature + Level, data = long_df, FUN = length); names(n_df)[3] <- "n"
    agg <- merge(agg, n_df, by = c("Feature", "Level"))
    feature_display <- setNames(names(heatmap_feature_choices), heatmap_feature_choices)
    agg$FeatureLabel <- factor(feature_display[agg$Feature], levels = feature_display[feats])
    agg$Level <- factor(agg$Level, levels = natural_level_order(agg$Level))
    p <- ggplot(agg, aes(x = Level, y = Shift, fill = Feature)) +
      ...
      scale_y_continuous(expand = expansion(mult = c(0.15, 0.18))) +
      facet_wrap(~ FeatureLabel, scales = "free_x", nrow = 1) +
      labs(x = NULL, y = "Mean methylation shift (Tumor \u2212 Normal)", subtitle = region_label_alus()) +
      theme_app()
    list(plot = p, data = agg[, c("Feature", "FeatureLabel", "Level", "Shift", "n")])
  })
  output$overview_composition_alus <- renderPlot({ overview_composition_build_alus()$plot })
  register_plot_export(output, "overview_composition_alus", function() overview_composition_build_alus()$plot, function() overview_composition_build_alus()$data)
  
  overview_sex_comparison_build_alus <- reactive({
    df <- overview_patient_shift_alus()
    sex_labels <- c("DONA" = "Female", "HOME" = "Male")
    df$Sex <- sex_labels[toupper(trimws(as.character(df$sexe)))]
    validate(need(any(!is.na(df$Sex)), "No sex information available for the current selection."))
    ...
    df$Sex <- factor(df$Sex, levels = intersect(c("Female", "Male"), unique(df$Sex)))
    n_by_sex <- table(df$Sex)
    df$SexLabel <- factor(paste0(as.character(df$Sex), " (n=", n_by_sex[as.character(df$Sex)], ")"), levels = paste0(names(n_by_sex), " (n=", n_by_sex, ")"))
    p <- ggplot(df, aes(x = SexLabel, y = shift, fill = Sex)) +
      geom_hline(yintercept = 0, color = "#7d92a3", linewidth = 0.4, linetype = "dashed") +
      geom_jitter(width = 0.08, size = 0.9, alpha = 0.3, color = "#16324f") +
      geom_boxplot(width = 0.45, outlier.shape = NA, alpha = 0.92, color = "#16324f", linewidth = 0.4) +
      stat_summary(fun = mean, geom = "point", shape = 23, size = 2.4, fill = "white", color = "#16324f", stroke = 0.8) +
      scale_fill_manual(values = c("Female" = "#d1495b", "Male" = "#3aa9c9")) +
      labs(x = NULL, y = "Methylation shift (Tumor \u2212 Normal)", subtitle = region_label_alus(), caption = "\u25c7 mean") +
      theme_app() + theme(legend.position = "none")
    if (length(levels(df$Sex)) == 2) {
      test_res <- run_group_test(df$shift[df$Sex == "Female"], df$shift[df$Sex == "Male"], method = "wilcox")
      y_max <- max(df$shift, na.rm = TRUE)
  ...
  output$overview_sex_comparison_alus <- renderPlot({ overview_sex_comparison_build_alus()$plot })
  register_plot_export(output, "overview_sex_comparison_alus", function() overview_sex_comparison_build_alus()$plot, function() overview_sex_comparison_build_alus()$data)
  
  # Alu 2. Genome Browser
  nav_alus <- reactiveValues(chr = NULL, start = NULL, end = NULL)
  
  observeEvent(input$chr_alus, { updateSelectInput(session, "browser_chr_alus", selected = input$chr_alus) })
  
  observeEvent(input$browser_chr_alus, {
    if (!identical(nav_alus$chr, input$browser_chr_alus)) {
      nav_alus$chr <- input$browser_chr_alus; nav_alus$start <- 1; nav_alus$end <- chrom_lengths[[input$browser_chr_alus]]
    }
    if (!identical(input$chr_alus, input$browser_chr_alus)) updateSelectInput(session, "chr_alus", selected = input$browser_chr_alus)
  }, ignoreInit = FALSE)
  
  observeEvent(input$prev_chr_alus, {
    idx <- match(nav_alus$chr, chrom_list)
    if (!is.na(idx) && idx > 1) updateSelectInput(session, "browser_chr_alus", selected = chrom_list[idx - 1])
    else showNotification("Already at the first chromosome.", type = "message", duration = 2)
  })
  observeEvent(input$next_chr_alus, {
    idx <- match(nav_alus$chr, chrom_list)
    if (!is.na(idx) && idx < length(chrom_list)) updateSelectInput(session, "browser_chr_alus", selected = chrom_list[idx + 1])
    else showNotification("Already at the last chromosome.", type = "message", duration = 2)
  })
  observeEvent(input$zoom_in_alus, { req(nav_alus$chr); v <- zoom_view(nav_alus$start, nav_alus$end, chrom_lengths[[nav_alus$chr]], factor = 0.5); nav_alus$start <- v$start; nav_alus$end <- v$end })
  observeEvent(input$zoom_out_alus, { req(nav_alus$chr); v <- zoom_view(nav_alus$start, nav_alus$end, chrom_lengths[[nav_alus$chr]], factor = 2); nav_alus$start <- v$start; nav_alus$end <- v$end })
  observeEvent(input$zoom_reset_alus, { req(nav_alus$chr); nav_alus$start <- 1; nav_alus$end <- chrom_lengths[[nav_alus$chr]] })
  
  observeEvent(input$browser_search_go_alus, {
    req(input$browser_search_alus)
    res <- parse_genomic_search_alus(input$browser_search_alus, alu_table, chrom_list, gene_lookup_table)
    if (identical(res$status, "ok")) {
      nav_alus$chr <- res$chr
      nav_alus$start <- max(1, floor(res$start)); nav_alus$end <- min(chrom_lengths[[res$chr]], ceiling(res$end))
      if (nav_alus$end <= nav_alus$start) nav_alus$end <- min(chrom_lengths[[res$chr]], nav_alus$start + 2e6)
      if (!identical(input$browser_chr_alus, res$chr)) updateSelectInput(session, "browser_chr_alus", selected = res$chr)
      showNotification(res$message, type = "message", duration = 4)
    } else {
      showNotification(res$message, type = "error", duration = 5)
    }
  ...
  observeEvent(event_data("plotly_click", source = "genome_overview_alus"), {
    ed <- event_data("plotly_click", source = "genome_overview_alus")
    req(ed, ed$customdata[1] %in% chrom_list)
    updateSelectInput(session, "browser_chr_alus", selected = ed$customdata[1])
  })
  
  output$browser_position_header_alus <- renderText({
    req(nav_alus$chr, nav_alus$start, nav_alus$end)
    paste0("Genome Browser (Alus) - chr", nav_alus$chr, ": ", format(round(nav_alus$start), big.mark = ",", scientific = FALSE), "-",
           format(round(nav_alus$end), big.mark = ",", scientific = FALSE), " (", format_mb_label(nav_alus$end - nav_alus$start), ")")
  })
  
  output$browser_external_links_alus <- renderUI({
    req(nav_alus$chr, nav_alus$start, nav_alus$end)
    region_links <- build_external_links(nav_alus$chr, nav_alus$start, nav_alus$end)
    div(style = "display:flex; gap:16px; flex-wrap:wrap; font-size:13px;",
        tags$a(bs_icon("box-arrow-up-right"), " UCSC Genome Browser", href = region_links$UCSC, target = "_blank", rel = "noopener noreferrer"),
        tags$a(bs_icon("box-arrow-up-right"), " Ensembl", href = region_links$Ensembl, target = "_blank", rel = "noopener noreferrer"),
        tags$a(bs_icon("box-arrow-up-right"), " TCGA", href = region_links$TCGA, target = "_blank", rel = "noopener noreferrer"))
  ...
  browser_view_alus <- reactive({
    req(nav_alus$chr, nav_alus$start, nav_alus$end)
    df <- alu_table[alu_table$chr == nav_alus$chr & alu_table$alu_end >= nav_alus$start & alu_table$alu_start <= nav_alus$end, ]
    df[order(df$alu_start), ]
  })
  
  output$genome_overview_plot_alus <- renderPlotly({
    df <- genome_wide_alus_plot_data
    plot_ly(df, x = ~genome_x, y = ~overall_meth, color = ~chr_parity, colors = c("#16324f", "#0e7c86"), customdata = ~as.character(chr),
           type = "scattergl", mode = "markers", marker = list(size = 3), text = ~alu_id, hoverinfo = "text", source = "genome_overview_alus") |>
      layout(showlegend = FALSE, xaxis = list(title = "", tickvals = as.numeric(chr_mid), ticktext = chrom_list, tickangle = 45),
             yaxis = list(title = "Mean methylation", range = c(0, 1), automargin = TRUE), shapes = chr_boundary_shapes,
             margin = list(t = 20, r = 20, b = 40, l = 50), autosize = TRUE) |>
      config(displayModeBar = FALSE, responsive = TRUE) |> event_register("plotly_click")
  })
  register_plotly_export(output, "genome_overview_plot_alus", function() genome_wide_alus_plot_data[, c("alu_id", "chr", "alu_start", "genome_x", "overall_meth")])
  
  browser_view_samples_alus <- reactive({
    alus_now <- browser_view_alus()
    req(nrow(alus_now) > 0)
    # ml_annot_alus doesn't carry alu_start/chr (dropped at preprocessing time to save memory
    df <- ml_annot_alus[ml_annot_alus$alu_id %in% alus_now$alu_id, c("sample_id", "patient_id", "Type", "alu_id", "methylation")]
    df$alu_id <- as.character(df$alu_id)
    df <- merge(df, alus_now[, c("alu_id", "alu_start")], by = "alu_id")
    df[df$Type %in% c("Tumor", "Normal"), ]
  })
  
  output$browser_chr_plot_alus <- renderPlotly({
    if (view_by_samples_alus()) {
      df <- browser_view_samples_alus()
      if (nrow(df) == 0) return(plotly_empty(type = "scatter", mode = "markers") |> layout(title = list(text = "No Alus with data in this region", font = list(size = 14))) |> config(displayModeBar = FALSE, responsive = TRUE))
      base_size <- max(3, min(8, 4000 / nrow(df))); hl_size <- base_size + 4
      p <- plot_ly(df, x = ~alu_start / 1e6)
      for (ty in c("Tumor", "Normal")) {
        sub <- df[df$Type == ty, ]; hl_sub <- sub$alu_id %in% input$bins_alus
        p <- add_trace(p, data = sub, x = ~alu_start / 1e6, y = ~methylation, type = "scattergl", mode = "markers", name = ty,
                       marker = list(color = if (ty == "Tumor") "#d1495b" else "#3aa9c9", size = ifelse(hl_sub, hl_size, base_size), opacity = 0.55, line = list(color = "#0b2436", width = ifelse(hl_sub, 1.2, 0))),
                       text = ~paste0(alu_id, "<br>Sample: ", sample_id, "<br>Patient: ", patient_id, "<br>", ty, ": ", round(methylation, 3)), hoverinfo = "text")
      }
      p |> layout(xaxis = list(title = paste0("Position on chr", nav_alus$chr, " (Mb)"), automargin = TRUE, rangeslider = list(visible = TRUE, thickness = 0.08, bgcolor = "#eef2f5", bordercolor = "#d8e0e6", borderwidth = 1)),
                  yaxis = list(title = "Methylation (per sample)", range = c(0, 1), automargin = TRUE),
                  legend = list(orientation = "h", x = 0.5, xanchor = "center", y = 1, yanchor = "bottom"), hovermode = "closest",
                  margin = list(t = 50, r = 20, b = 50, l = 55), autosize = TRUE) |> config(displayModeBar = FALSE, responsive = TRUE)
    } else {
      df <- browser_view_alus()
      if (nrow(df) == 0) return(plotly_empty(type = "scatter", mode = "markers") |> layout(title = list(text = "No Alus in this region", font = list(size = 14))) |> config(displayModeBar = FALSE, responsive = TRUE))
      hl <- df$alu_id %in% input$bins_alus
      base_size <- max(3, min(9, 3000 / nrow(df))); hl_size <- base_size + 5
      plot_ly(df, x = ~alu_start / 1e6) |>
        add_trace(y = ~mean_methylation_tumor, type = "scattergl", mode = "markers", name = "Tumor",
                  marker = list(color = "#d1495b", size = ifelse(hl, hl_size, base_size), line = list(color = "#0b2436", width = ifelse(hl, 1.5, 0))),
                  text = ~paste0(alu_id, "<br>Tumor mean: ", round(mean_methylation_tumor, 3)), hoverinfo = "text") |>
        add_trace(y = ~mean_methylation_normal, type = "scattergl", mode = "markers", name = "Normal",
                  marker = list(color = "#3aa9c9", size = ifelse(hl, hl_size, base_size), line = list(color = "#0b2436", width = ifelse(hl, 1.5, 0))),
                  text = ~paste0(alu_id, "<br>Normal mean: ", round(mean_methylation_normal, 3)), hoverinfo = "text") |>
        layout(xaxis = list(title = paste0("Position on chr", nav_alus$chr, " (Mb)"), automargin = TRUE, rangeslider = list(visible = TRUE, thickness = 0.08, bgcolor = "#eef2f5", bordercolor = "#d8e0e6", borderwidth = 1)),
               yaxis = list(title = "Mean methylation", range = c(0, 1), automargin = TRUE),
               legend = list(orientation = "h", x = 0.5, xanchor = "center", y = 1, yanchor = "bottom"), hovermode = "closest",
               margin = list(t = 50, r = 20, b = 50, l = 55), autosize = TRUE) |> config(displayModeBar = FALSE, responsive = TRUE)
    }
  })
  register_plotly_export(output, "browser_chr_plot_alus", function() {
    if (view_by_samples_alus()) browser_view_samples_alus() else browser_view_alus()[, c("alu_id", "chr", "alu_start", "mean_methylation_tumor", "mean_methylation_normal")]
  })
  
  # Alu 3. Genome-wide Profile
  output$manhattan_plot_alus <- renderPlotly({
    df <- genome_wide_alus_plot_data
    validate(need(nrow(df) > 0, "No genome-wide methylation data available."))
    cat_colors <- c("Not significant" = "#168AAD", "Extreme outlier" = "#F4A261", "Significant hypomethylation" = "#76C893", "Significant hypermethylation" = "#8D99AE")
    plot_ly(df, x = ~genome_x, y = ~tumor_normal_diff, color = ~manhattan_category, colors = cat_colors, type = "scattergl", mode = "markers",
           marker = list(size = 4, opacity = 0.85, line = list(width = 0)),
           text = ~paste0("Alu: ", alu_id, "<br>Chr: ", as.character(chr), "<br>Diff (Tumor \u2212 Normal): ", round(tumor_normal_diff, 3),
                          "<br>q-value: ", ifelse(is.na(q_value), "NA", format(signif(q_value, 3), scientific = TRUE)),
                          "<br>Patients tested: ", ifelse(is.na(n), 0, n)), hoverinfo = "text") |>
      layout(xaxis = list(title = "", tickvals = as.numeric(chr_mid), ticktext = chrom_list, tickangle = 45),
             yaxis = list(title = "Methylation difference (Tumor \u2212 Normal)", zeroline = TRUE, zerolinewidth = 1.4, zerolinecolor = "#16324f"),
             shapes = chr_boundary_shapes, legend = list(orientation = "h", x = 0, y = 1.12), hovermode = "closest") |>
      config(displayModeBar = TRUE, toImageButtonOptions = list(format = "png", filename = "genome_wide_methylation_profile_alus"))
  })
  register_plotly_export(output, "manhattan_plot_alus", function() genome_wide_alus_plot_data[, c("alu_id", "chr", "alu_start", "tumor_normal_diff", "q_value", "n", "manhattan_category")])
  
  # Alu 4. Feature x Chromosome Heatmap
  max_dendro_alus <- 300
  heatmap_zoom_pct_alus <- reactive({ z <- input$heatmap_zoom_alus; if (is.null(z) || !is.finite(z)) 100 else z })
  observeEvent(input$heatmap_zoom_reset_alus, { updateSliderInput(session, "heatmap_zoom_alus", value = 100) })
  
  heatmap_build_alus <- reactive({
    feature <- input$heatmap_feature_alus; if (is.null(feature)) feature <- "none"
    alus_now <- selected_alus()
    validate(need(length(alus_now) > 0, "Select at least one Alu in the sidebar."))
    shift_df <- patient_alu_shift[as.character(patient_alu_shift$alu_id) %in% alus_now, ]
    validate(need(nrow(shift_df) > 0, "No patients have both a Tumor and a Normal value at any of the selected Alus. Try selecting more Alus."))
    max_abs <- max(abs(shift_df$shift), na.rm = TRUE)
    alus_present <- alu_table$alu_id[alu_table$alu_id %in% unique(as.character(shift_df$alu_id))]
    mat <- build_shift_matrix(shift_df, alus_present, id_col = "alu_id")
    mat[is.na(mat)] <- 0
    validate(need(nrow(mat) >= 2, "Need at least two patients with data to build a clustered heatmap."))
    row_hc <- hclust(dist(mat)); row_order <- rownames(mat)[row_hc$order]; row_segments <- hclust_to_segments(row_hc); n_patients <- length(row_order)
    show_col_dendro <- ncol(mat) >= 2 && ncol(mat) <= max_dendro_alus
    if (show_col_dendro) {
      col_hc <- hclust(dist(t(mat))); col_order <- colnames(mat)[col_hc$order]; col_segments <- hclust_to_segments(col_hc)
    } else {
      col_order <- alus_present[alus_present %in% colnames(mat)]; col_segments <- NULL
    }
    n_alus <- length(col_order)
    shift_df$alu_id <- factor(as.character(shift_df$alu_id), levels = col_order)
    shift_df$patient_id <- factor(shift_df$patient_id, levels = row_order)
    shift_df$x <- as.integer(shift_df$alu_id); shift_df$y <- as.integer(shift_df$patient_id)
    list(feature = feature, shift_df = shift_df, max_abs = max_abs, row_order = row_order, col_order = col_order,
         n_patients = n_patients, n_bins = n_alus, row_segments = row_segments, col_segments = col_segments, show_col_dendro = show_col_dendro)
  })
  
  feature_heatmap_build_alus <- reactive({
    d <- heatmap_build_alus()
    p_main <- ggplot(d$shift_df, aes(x = x, y = y, fill = shift)) +
      geom_tile(color = "white", linewidth = 0.15) +
      scale_fill_gradient2(low = "#3aa9c9", mid = "white", high = "#d1495b", midpoint = 0, limits = c(-d$max_abs, d$max_abs), name = "Methylation\nshift\n(Tumor \u2212 Normal)") +
      scale_x_continuous(breaks = seq_len(d$n_bins), labels = d$col_order, expand = c(0, 0)) +
      scale_y_continuous(breaks = seq_len(d$n_patients), labels = d$row_order, expand = c(0, 0)) +
      labs(x = if (d$show_col_dendro) "Alu (dendrogram-ordered)" else "Alu (genome order; too many to cluster)", y = "Patient (dendrogram-ordered)") +
      theme_minimal(base_size = 12) + theme(panel.grid = element_blank(), axis.text.y = element_text(size = 8), axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 7))
    row_dendro <- plot_dendrogram(d$row_segments, orientation = "left") + scale_y_continuous(limits = c(0.5, d$n_patients + 0.5), expand = c(0, 0))
    col_dendro <- if (d$show_col_dendro) plot_dendrogram(d$col_segments, orientation = "top") + scale_x_continuous(limits = c(0.5, d$n_bins + 0.5), expand = c(0, 0)) else plot_spacer()
    heatmap_export_data <- d$shift_df[, c("patient_id", "alu_id", "chr", "meth_tumor", "meth_normal", "shift")]
    if (identical(d$feature, "none")) {
      design <- "
      AB
      CD
      "
      p_none <- plot_spacer() + col_dendro + row_dendro + p_main + patchwork::plot_layout(design = design, widths = c(1, 8), heights = c(1, 8))
      return(list(plot = p_none, data = heatmap_export_data))
    }
    ann_df <- patient_annotation[patient_annotation$patient_id %in% d$row_order, ]
    ann_df$patient_id <- factor(ann_df$patient_id, levels = d$row_order); ann_df$y <- as.integer(ann_df$patient_id); ann_df$x <- "Feature"
    ann_df$value <- as.character(ann_df[[d$feature]]); ann_df$value[is.na(ann_df$value)] <- "unknown"
    levels_cat <- sort(unique(ann_df$value)); base_palette <- c("#0e7c86", "#d1495b", "#3aa9c9", "#e0a339", "#2fae66", "#16324f", "#7d92a3")
    pal <- setNames(base_palette[seq_along(levels_cat)], levels_cat)
    feature_label <- names(heatmap_feature_choices)[heatmap_feature_choices == d$feature]
    p_ann <- ggplot(ann_df, aes(x = x, y = y, fill = value)) + geom_tile(color = "white", linewidth = 0.15) + scale_fill_manual(values = pal, name = feature_label) +
      scale_y_continuous(limits = c(0.5, d$n_patients + 0.5), expand = c(0, 0)) + labs(x = NULL, y = NULL) + theme_minimal(base_size = 12) +
      theme(axis.text.y = element_blank(), axis.text.x = element_blank(), panel.grid = element_blank())
    design <- "
    AAB
    CDE
    "
    p_feat <- plot_spacer() + col_dendro + row_dendro + p_ann + p_main + patchwork::plot_layout(design = design, widths = c(1, 0.6, 8), heights = c(1, 8))
    list(plot = p_feat, data = heatmap_export_data)
  })
  
  output$feature_heatmap_alus <- renderPlot({ feature_heatmap_build_alus()$plot },
    height = function() { d <- heatmap_build_alus(); natural_h <- max(300, round(d$n_patients * 20 + 160)); max(150, round(natural_h * heatmap_zoom_pct_alus() / 100)) },
    width  = function() { d <- heatmap_build_alus(); natural_w <- max(400, round(d$n_bins * 14 + 160)); max(200, round(natural_w * heatmap_zoom_pct_alus() / 100)) })
  register_plot_export(output, "feature_heatmap_alus", function() feature_heatmap_build_alus()$plot, function() feature_heatmap_build_alus()$data, width = 12, height = 9)
  
# Alu 5. Clinical Explorer
  filtered_clinical_profile_alus <- reactive({ apply_clinical_filters(clinical_profile_table, clinical_numeric_cols, clinical_categorical_cols, input, prefix = "clinfilter_alus_") })
  
  observeEvent(input$clinfilter_reset_alus, {
    for (col in clinical_categorical_cols) updateCheckboxGroupInput(session, paste0("clinfilter_alus_", col), selected = clinical_categorical_choices(clinical_profile_table[[col]]))
    for (col in clinical_numeric_cols) { vals <- clinical_profile_table[[col]]; vals <- vals[is.finite(vals)]; rng <- if (length(vals) > 0) range(vals) else c(0, 1); updateSliderInput(session, paste0("clinfilter_alus_", col), value = rng) }
  })
  
  output$clinical_table_count_badge_alus <- renderUI({
    n <- nrow(filtered_clinical_profile_alus()); total <- nrow(clinical_profile_table)
    span(style = "font-size:12px; font-weight:600; color:#16324f; background:#eef2f5; border:1px solid #d8e0e6; border-radius:20px; padding:4px 12px; white-space:nowrap;",
         paste0(format(n, big.mark = ","), " of ", format(total, big.mark = ","), " patients"))
  })
  
  clinical_table_data_alus <- reactive({ filtered_clinical_profile_alus() })
  clinical_table_proxy_alus <- dataTableProxy("clinical_table_alus")
  selected_patient_id_alus <- reactive({
    sel <- input$clinical_table_alus_rows_selected
    if (is.null(sel) || length(sel) == 0) return(NULL)
    as.character(clinical_table_data_alus()$patient_id[sel])
  })
  observeEvent(input$clear_patient_selection_alus, { selectRows(clinical_table_proxy_alus, NULL) })
  
  clinical_boxplot_build_alus <- reactive({
    gene <- input$mutation_gene_alus; req(gene)
    test_method <- input$mutation_stat_test_alus; if (is.null(test_method)) test_method <- "wilcox"
    status_labels <- c("0" = "Wild-type", "1" = "Mutant")
    filtered_ids <- filtered_clinical_profile_alus()$patient_id
    alus_now <- selected_alus()
    validate(need(length(alus_now) > 0, "No Alus selected. Choose Alus in the sidebar."))
    
    if (view_by_samples_alus()) {
      validate(need(gene %in% names(patient_annotation), paste0(gene, " status is not available in this dataset.")))
      df <- ml_annot_alus[ml_annot_alus$alu_id %in% alus_now & ml_annot_alus$Type == "Tumor", c("sample_id", "patient_id", "alu_id", "methylation")]
      df <- df[df$patient_id %in% filtered_ids, ]
      df <- merge(df, patient_annotation[, c("patient_id", gene)], by = "patient_id")
      names(df)[names(df) == gene] <- "GeneStatus"
      status_raw <- as.character(df$GeneStatus)
      ...
      df <- df[!is.na(df$Status) & is.finite(df$Value), ]
      validate(need(nrow(df) > 0, "No patients match the current filters. Adjust the filters above."))
      validate(need(length(unique(df$Status)) == 2, paste0("No data at the current Alu selection for at least one of the two ", gene, " groups (Wild-type/Mutant). Try selecting more Alus.")))
      y_lab <- "Tumor methylation (per sample \u00d7 Alu)"
    } else {
      df <- overview_patient_shift_alus()
      validate(need(!is.null(df) && nrow(df) > 0, "No data available for the current Alu selection."))
      validate(need(gene %in% names(df), paste0(gene, " status is not available in this dataset.")))
      df <- df[df$patient_id %in% filtered_ids, ]
      validate(need(nrow(df) > 0, "No patients match the current filters. Adjust the filters above."))
      status_raw <- as.character(df[[gene]])
      ...
      df <- df[!is.na(df$Status) & is.finite(df$Value), ]
      validate(need(length(unique(df$Status)) == 2, paste0("No data at the current Alu selection for at least one of the two ", gene, " groups (Wild-type/Mutant). Try selecting more Alus.")))
      y_lab <- "Mean tumor methylation (per Alu)"
    }
    df$Status <- factor(df$Status, levels = c("Wild-type", "Mutant"))
    n_by_status <- table(df$Status)
    test_res <- run_group_test(df$Value[df$Status == "Mutant"], df$Value[df$Status == "Wild-type"], method = test_method)
    ...
    p <- ggplot(df, aes(x = Status, y = Value, fill = Status)) +
      ...
      annotate("text", x = 1.5, y = y_max * 1.1, label = test_res$label, size = 3.5, color = "#16324f", fontface = "bold") +
      scale_y_continuous(limits = c(0, max(1, y_max * 1.18)), expand = expansion(mult = c(0.02, 0.02))) +
      labs(x = NULL, y = y_lab, title = paste0(gene, " mutation status vs. tumor methylation (Alus)"), subtitle = region_label_alus()) +
      theme_app() + theme(legend.position = "none")
    sel_id <- selected_patient_id_alus()
    if (!is.null(sel_id) && sel_id %in% df$patient_id) {
      hl <- df[df$patient_id == sel_id, ]
      p <- p + geom_point(data = hl, aes(x = Status, y = Value), shape = 21, size = 4.8, fill = "#e0a339", color = "#16324f", stroke = 1.1) +
        geom_text(data = hl, aes(x = Status, y = Value, label = paste0("Patient ", patient_id)), vjust = -1.3, size = 3, color = "#16324f", fontface = "bold")
    }
    export_cols <- intersect(c("patient_id", "sample_id", "alu_id", "Status", "Value", "Normal", "shift"), names(df))
    list(plot = p, data = df[, export_cols])
  })
  output$clinical_boxplot_alus <- renderPlot({ clinical_boxplot_build_alus()$plot })
  register_plot_export(output, "clinical_boxplot_alus", function() clinical_boxplot_build_alus()$plot, function() clinical_boxplot_build_alus()$data)
  
  output$clinical_table_alus <- renderDT({
    df <- clinical_table_data_alus()
    display_df <- df
    names(display_df) <- vapply(names(df), clinical_label_for, character(1))
    num_cols <- vapply(display_df, is.numeric, logical(1))
    display_df[num_cols] <- lapply(display_df[num_cols], round, 3)
  ...
  output$selected_patient_banner_alus <- renderUI({
    sel_id <- selected_patient_id_alus()
    if (is.null(sel_id)) {
      return(div(style = "font-size:12px; color:#7d92a3; background:#eef2f5; border:1px dashed #c3ccd3; border-radius:8px; padding:10px 16px; margin-bottom:14px;",
                bs_icon("info-circle"), " No patient selected, click a row in the clinical metadata table to highlight that patient in the plot."))
    ...
    chip_cols <- intersect(c("sexe", "estadi2", "MSS", "BRAF", "KRAS", "TP53", "Recaiguda", "MethShift"), names(row))
    div(style = "display:flex; align-items:center; gap:26px; flex-wrap:wrap; background:#eef2f5; border:1px solid #d8e0e6; border-radius:8px; padding:12px 18px; margin-bottom:14px;",
        div(style = "display:flex; align-items:center; gap:8px; font-weight:700; color:#16324f;", bs_icon("person-check-fill", size = "1.3em"), paste0("Patient ", sel_id)),
        lapply(chip_cols, chip),
        actionButton("clear_patient_selection_alus", "Clear selection", icon = bs_icon("x-circle"), class = "btn-sm btn-outline-secondary", style = "margin-left:auto;"))
  })
  
# Alu 6. Alu Table
  output$alutable_gene_note <- renderUI({
    if (!gene_annotation_available_alus) {
      return(div(style = "font-size:12px; color:#a34; background:#fdf1ef; border:1px solid #f0d6d1; border-radius:8px; padding:8px 14px; max-width:560px;",
                bs_icon("exclamation-triangle"), " Gene annotation source files were not found when the app started (needs the same COSMIC reference file as the Bin Table)."))
    }
  })
  output$alutable_promoter_note <- renderUI({
    if (!promoter_annotation_available_alus) {
      return(div(style = "font-size:12px; color:#a34; background:#fdf1ef; border:1px solid #f0d6d1; border-radius:8px; padding:8px 14px; max-width:560px;",
                bs_icon("exclamation-triangle"), " Promoter annotation source file was not found when the app started."))
    }
  })
  ...
  output$alutable <- renderDT({
    display_df <- build_display_bin_table(filtered_alu_table(), columns = alu_table_columns)
    meth_cols <- intersect(c("Tumor Mean", "Normal Mean", "Tumor SD", "Normal SD", "\u0394 (Tumor \u2212 Normal)"), names(display_df))
    count_cols <- intersect(c("Tumor Samples", "Normal Samples", "Gene Count"), names(display_df))
    pct_cols <- intersect(c("Prevalence %"), names(display_df))
    delta_col <- "\u0394 (Tumor \u2212 Normal)"
    links_col <- which(names(display_df) == "Quick Links")
    dt <- datatable(display_df, container = alu_table_header_sketch, rownames = FALSE, class = "stripe hover compact",
                    escape = if (length(links_col) > 0) -links_col else TRUE,
                    options = list(scrollX = TRUE, pageLength = 15, lengthMenu = list(c(10, 15, 25, 50, -1), c("10", "15", "25", "50", "All")), dom = "ltip",
                                  language = list(search = "Quick search:", lengthMenu = "Show _MENU_ Alus per page"),
                                  columnDefs = list(list(className = "dt-center", targets = "_all"))))
    if (length(meth_cols) > 0) dt <- formatRound(dt, meth_cols, 3)
    if (length(pct_cols) > 0) dt <- formatRound(dt, pct_cols, 3)
    if (length(count_cols) > 0) dt <- formatRound(dt, count_cols, 0)
    if (delta_col %in% names(display_df)) dt <- formatStyle(dt, delta_col, fontWeight = "600", color = styleInterval(0, c("#0e7c86", "#d1495b")))
    if ("Gene Count" %in% names(display_df)) dt <- formatStyle(dt, "Gene Count", fontWeight = styleInterval(0, c("normal", "700")), color = styleInterval(0, c("#adb8c0", "#16324f")), backgroundColor = styleInterval(0, c("transparent", "#e8f5f3")))
    dt
  })
  output$alutable_download <- downloadHandler(filename = function() {"alutable.csv"}, content = function(file) {
    write.csv(build_display_bin_table(filtered_alu_table(), columns = alu_table_columns, plain = TRUE), file, row.names = FALSE)
  })
  
# Alu 7. Tumor vs. Normal
  network_sample_matrix_alus <- reactive({
    alus_now <- selected_alus()
    validate(need(length(alus_now) > 0, "No Alus selected. Choose Alus in the sidebar."))
    df <- ml_annot_alus[ml_annot_alus$alu_id %in% alus_now, c("sample_id", "alu_id", "methylation")]
    mat <- build_wide_matrix(df, "sample_id", "alu_id", "methylation")
    validate(need(!is.null(mat), "Not enough overlapping data across samples for the selected Alus to build a similarity network. Try selecting more Alus."))
    keep_cols <- apply(mat, 2, function(x) stats::sd(x) > 0)
    mat <- mat[, keep_cols, drop = FALSE]
    validate(need(nrow(mat) >= 3 && ncol(mat) >= 2, "Not enough overlapping data across samples for the selected Alus to build a similarity network. Try selecting more Alus."))
    mat
  })
  
  tn_density_build_alus <- reactive({
    if (view_by_samples_alus()) {
      df <- ml_annot_alus[ml_annot_alus$alu_id %in% selected_alus() & ml_annot_alus$Type %in% c("Tumor", "Normal"), c("Type", "methylation")]
      validate(need(nrow(df) > 0, "No Alus selected. Choose Alus in the sidebar."))
      plot_df <- data.frame(Type = df$Type, Methylation = df$methylation)
      plot_df <- plot_df[is.finite(plot_df$Methylation), ]
      validate(need(nrow(plot_df) > 0, "No valid methylation values for the current selection."))
      x_lab <- "Methylation (per sample \u00d7 Alu)"
    } else {
      df <- selected_alu_table()
      validate(need(nrow(df) > 0, "No Alus selected. Choose Alus in the sidebar."))
      plot_df <- data.frame(Type = rep(c("Tumor", "Normal"), each = nrow(df)), Methylation = c(df$mean_methylation_tumor, df$mean_methylation_normal))
      plot_df <- plot_df[is.finite(plot_df$Methylation), ]
      validate(need(nrow(plot_df) > 0, "No valid methylation values for the current selection."))
      x_lab <- "Mean methylation (per Alu)"
    }
    p <- ggplot(plot_df, aes(x = Methylation, fill = Type, color = Type)) +
      geom_density(alpha = 0.35, linewidth = 0.9, na.rm = TRUE) +
      scale_fill_manual(values = c("Tumor" = "#d1495b", "Normal" = "#3aa9c9")) +
      scale_color_manual(values = c("Tumor" = "#d1495b", "Normal" = "#3aa9c9")) +
      labs(x = x_lab, y = "Density", subtitle = region_label_alus()) +
      theme_app()
    list(plot = p, data = plot_df)
  })
  output$tn_density_alus <- renderPlot({ tn_density_build_alus()$plot })
  register_plot_export(output, "tn_density_alus", function() tn_density_build_alus()$plot, function() tn_density_build_alus()$data)
  
  tn_network_build_alus <- reactive({
    mat <- network_sample_matrix_alus()
    features <- input$network_color_feature_alus
    features <- intersect(features, names(sample_annotation))
    if (length(features) == 0) features <- "Type"
    corr_threshold <- input$network_corr_threshold_alus
    if (is.null(corr_threshold) || !is.finite(corr_threshold)) corr_threshold <- 0.7
    ...
    if (length(ids) >= 2) {
      pairs <- utils::combn(ids, 2)
      edge_df <- data.frame(from = pairs[1, ], to = pairs[2, ], corr = cor_mat[cbind(pairs[1, ], pairs[2, ])], stringsAsFactors = FALSE)
      edge_df <- edge_df[is.finite(edge_df$corr) & edge_df$corr >= corr_threshold, ]
      if (nrow(edge_df) > 0) {
        edge_df <- merge(edge_df, setNames(nodes_base[, c("sample_id", "x", "y")], c("from", "x_from", "y_from")), by = "from")
        edge_df <- merge(edge_df, setNames(nodes_base[, c("sample_id", "x", "y")], c("to", "x_to", "y_to")), by = "to")
    ...
    validate(need(nrow(nodes_base) > 0, "No samples available for the current Alu selection."))
    type_shapes <- c("Tumor" = 21, "Normal" = 24, "unknown" = 22)
    make_network_panel <- function(feature, show_subtitle) {
      nodes <- nodes_base
      nodes$color_label <- as.character(nodes[[feature]]); nodes$color_label[is.na(nodes$color_label)] <- "unknown"
      color_palette <- categorical_palette(nodes$color_label)
      feature_label <- names(network_color_choices)[network_color_choices == feature][1]
      p <- ggplot()
      if (!is.null(edge_df) && nrow(edge_df) > 0) {
        p <- p + geom_segment(data = edge_df, aes(x = x_from, y = y_from, xend = x_to, yend = y_to, alpha = corr), color = "#7d92a3", linewidth = 0.4, show.legend = FALSE)
      }
      p + geom_point(data = nodes, aes(x = x, y = y, fill = color_label, shape = Type), size = 5, color = "#0b2436", stroke = 0.6) +
        geom_text(data = nodes, aes(x = x, y = y, label = patient_id), vjust = -1.3, size = 2.8, color = "#16324f") +
        scale_fill_manual(values = color_palette, name = feature_label) +
        ...
        guides(fill = guide_legend(override.aes = list(shape = 21))) +
        labs(x = "Dimension 1", y = "Dimension 2",
             title = if (length(features) > 1) feature_label else "Patient similarity network (Alus, methylation correlation)",
             subtitle = if (show_subtitle) paste0(region_label_alus(), " \u2014 edges at corr \u2265 ", corr_threshold) else NULL) +
        theme_minimal(base_size = 12) + theme(legend.position = "bottom", legend.box = "vertical", legend.margin = margin(t = 0, b = 0), panel.grid.minor = element_blank())
    }
    p <- if (length(features) == 1) {
      make_network_panel(features[1], show_subtitle = TRUE)
    } else {
      ...
      n_col <- if (length(panels) >= 3) 2 else length(panels)
      patchwork::wrap_plots(panels, ncol = n_col, guides = "collect") +
        patchwork::plot_annotation(title = "Patient similarity network (Alus, methylation correlation)",
                                    subtitle = paste0(region_label_alus(), " \u2014 edges at corr \u2265 ", corr_threshold, "; circle = Tumor, triangle = Normal"),
                                    theme = theme(plot.title = element_text(face = "bold", size = 13, color = "#16324f"), plot.subtitle = element_text(size = 10, color = "#7d92a3"))) &
        theme(legend.position = "bottom")
    }
    node_export <- nodes_base[, intersect(c("sample_id", "patient_id", "Type", "x", "y", features), names(nodes_base))]
    list(plot = p, data = node_export)
  })
  output$tn_network_alus <- renderPlot({ tn_network_build_alus()$plot })
  register_plot_export(output, "tn_network_alus", function() tn_network_build_alus()$plot, function() tn_network_build_alus()$data, width = 10, height = 7)
}
